...
export const EmptyState = ({ onClearFilters }) => {
	...
	return (
		<div className="flex flex-col items-center justify-center py-24 px-4 text-center bg-surface border border-border/40 rounded-3xl w-full my-8">
			<div className="w-24 h-24 bg-primary/10 rounded-full flex items-center justify-center mb-6 relative shadow-inner text-primary">
				<PackageSearch className="w-12 h-12" strokeWidth={1.75} />
			</div>
			...
			<p className="text-text-secondary max-w-sm mb-8 text-base">
				{isRtl 
					? "عذراً، لا توجد منتجات تطابق الفلاتر الحالية. جرب تغيير خيارات البحث أو مسح الفلاتر." 
					: "Sorry, no products match your current filters. Try changing search options or clearing filters."}
			</p>
